...
class MasterServer(object):
    ...
    def register_server(self, name, port, map, maxplayers, pwprotected, gamemode):
        if self.server_exists(cherrypy.request.remote.ip, port):
            self.internal_update_server(cherrypy.request.remote.ip, port, name, map, 0, maxplayers, pwprotected,
                                        gamemode)
            return json.dumps({'error': False,
                               'message': 'Sucessfully updated your server [%s %s:%s] on the server browser.' % (
                               name, cherrypy.request.remote.ip, port)})
        else:
            server = Server()
            ...
            try:
                ...
                if (cherrypy.request.remote.ip != "127.0.0.1"):
                    return json.dumps({'error': True,
                                       'message': 'Unable to connect to server [%s %s:%s]. Please verify your ports are forwarded and your firewall is not blocking the game. Your server will not be visible in the Server Browser.' % (
            ...
            return json.dumps({'error': False,
                               'message': 'Sucessfully added your server [%s %s:%s] to the server browser.' % (
                               name, cherrypy.request.remote.ip, port), 'heartbeat': self.time_between_heartbeats})
    ...
    def update_server(self, port, name, map, playercount, maxplayers, pwprotected, gamemode):
        if self.internal_update_server(cherrypy.request.remote.ip, port, name, map, playercount, maxplayers,
                                       pwprotected, gamemode):
            return json.dumps({'error': False,
                               'message': 'Sucessfully updated your server [%s %s:%s] on the server browser.' % (
                               name, cherrypy.request.remote.ip, port)})
    ...
    def get_serverlist(self):
        self.returnlist = []
        for server in self.serverlist:
            jsonstring = {'name': server.name, 'port': server.port, 'map': server.map,
                          'playercount': server.playercount, 'maxplayers': server.maxplayers,
                          'pwprotected': server.pwprotected, 'gamemode': server.gamemode, 'ip': server.ip}
